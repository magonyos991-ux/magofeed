...
//   node promo/render-demo.mjs --smooth     -> sortie 60 i/s (interpolation, ~30 min)
...
// --smooth recalcule les images intermédiaires pour sortir en 60 i/s : c'est
// nettement plus doux sur les défilements, mais comptez ~30 min de calcul.
...
// L'enregistrement de Playwright capture en pixels CSS : on prend donc une
// fenêtre large et un zoom CSS, ce qui laisse l'app se composer sur 390 px de
// large — un gabarit d'iPhone — tout en filmant en haute définition.
//
// La largeur de capture décide de la FLUIDITÉ, pas de la netteté : sur cette
// machine (rendu logiciel), un défilement réel de l'accueil tourne à ~30 i/s
// en 810 px de large et s'effondre à ~4 i/s en 1080. On filme donc en 810 et
// on agrandit en 1080 au montage : à l'œil le texte est identique, et il y a
// sept fois plus d'images. Mesure : node promo/render-demo.mjs (voir README).
const PHONE = { width: 390, height: 844 };           // gabarit iPhone
const CAPTURE_W = Number(process.env.MAGOFEED_CAPTURE_W || 810);
const OUTPUT_W = 1080;                               // largeur du fichier livré
const ZOOM = CAPTURE_W / PHONE.width;
const VIEW = { width: CAPTURE_W, height: Math.round(PHONE.height * ZOOM) };
...
const TAP_CSS = `
  html{zoom:__ZOOM__}
  /* Le zoom CSS ne s'applique pas aux unités de viewport : l'app se croit dans
     une fenêtre de __VH__ px de haut et sa carte « plein écran » déborde
     largement. On lui redonne les proportions d'un vrai téléphone — le cadre
     arrondi et sa marge, eux, viennent de l'app (body{padding:20px}). */
  body{min-height:__PH__px!important}
  #app{max-height:calc(__PH__px - 40px)!important}
  #map-box.map-fullscreen{position:absolute!important;inset:0!important;width:auto!important;height:auto!important}
...
async function main() {
  ...
  }, { stores, center: CENTER, radiusKm: CACHE_RADIUS_KM, pseudo: PSEUDO, css: TAP_CSS.replace('__ZOOM__', String(ZOOM)).replace(/__PW__/g, String(PHONE.width))
      .replace(/__PH__/g, String(PHONE.height)).replace(/__VH__/g, String(VIEW.height)), zoom: ZOOM });
  ...
  async function scroll(dy, ms = 1100, at = { x: 195, y: 460 }) {
    const moved = await page.evaluate(([x, y, dy, ms, zoom]) => new Promise(done => {
      const scrollable = el => {
        const st = getComputedStyle(el);
        return /(auto|scroll)/.test(st.overflowY) && el.scrollHeight > el.clientHeight + 4;
      };
      // Sous zoom CSS, les coordonnées client sont en pixels de la FENÊTRE :
      // on convertit, sinon le point tombe à côté du conteneur visé.
      let el = document.elementFromPoint(x * zoom, y * zoom), box = null;
      while (el && el !== document.body) { if (scrollable(el)) { box = el; break; } el = el.parentElement; }
      if (!box) {   // filet : le plus grand conteneur défilable de l'app
        const app = document.getElementById('app') || document.body;
        box = [...app.querySelectorAll('*')].filter(scrollable)
          .sort((a, b) => b.scrollHeight - a.scrollHeight)[0];
      }
      if (!box) return done(false);
      const from = box.scrollTop, t0 = performance.now();
      // départ et arrivée très doux : moins de déplacement par image aux
      // extrémités, c'est là que l'œil accroche.
      const ease = t => t < .5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2;
      (function step() {
        const t = Math.min(1, (performance.now() - t0) / ms);
        box.scrollTop = from + dy * ease(t);
        if (t < 1) requestAnimationFrame(step); else done(box.scrollTop !== from);
      })();
    }), [at.x, at.y, dy, ms, ZOOM]);
    if (!moved) console.log('  !! rien à faire défiler ici');
  }

  // Mouvement de caméra : un seul vol plané (dézoom + déplacement combinés),
  // animé par Leaflet. Plus lisible qu'un dézoom suivi d'un glissement, et
  // plus doux qu'un glissement piloté cran par cran depuis Node.
  async function flyMap({ dz = 0, dx = 0, dy = 0, ms = 2000 }) {
    await page.evaluate(([dz, dx, dy, ms]) => {
      const m = window.exploreMap || window._leafletMap;
      if (!m) return;
      const p = m.project(m.getCenter(), m.getZoom()).add([dx, dy]);
      m.flyTo(m.unproject(p, m.getZoom()), m.getZoom() + dz, { duration: ms / 1000, easeLinearity: 0.2 });
    }, [dz, dx, dy, ms]);
    await wait(ms + 400);
  }
  ...
  await wait(9000);
  // #explore-map-host est posé sur le <body> en position fixe : on le recale
  // sur le cadre de l'app, mesuré une fois la mise en page faite.
  await page.evaluate(zoom => {
    const a = document.getElementById('app');
    if (!a) return;
    const r = a.getBoundingClientRect();
    const st = document.createElement('style');
    st.textContent = `#explore-map-host{position:fixed!important;left:${r.left / zoom}px!important;` +
      `top:${r.top / zoom}px!important;width:${r.width / zoom}px!important;` +
      `height:${r.height / zoom}px!important;right:auto!important;bottom:auto!important}`;
    document.head.appendChild(st);
  }, ZOOM);                              // chargement + position GPS
  const t0 = Date.now();                         // début réel de la démo filmée

  // 1. l'accueil — on ne s'attarde pas, on sait ce qu'on vient chercher
  await wait(1700); await shot('accueil');
  await scroll(340, 1200); await wait(700); await shot('accueil-defile');
  await scroll(-340, 900); await wait(400);

  // 2. la boisson qu'on cherche
  await tap('#t-placeholder', { pause: 1100 }); await shot('recherche');
  await page.locator('#q').click({ force: true });
  await page.locator('#q').type(QUERY, { delay: 170 });
  await wait(1400); await shot('suggestions');

  // 3. la carte des magasins qui l'ont, en un seul mouvement de caméra
  await tapText(DRINK, { pause: 5200 }); await shot('carte');
  await flyMap({ dz: -2, dx: 60, dy: -40, ms: 2400 }); await wait(700); await shot('carte-large');

  // 4. la fiche : on la met en favori au passage
  await tapText('voir la fiche', { pause: 2600 }); await shot('fiche');
  await tap('#res-fav', { pause: 1300 }); await shot('favori');

  // 5. on confirme le stock du magasin le plus proche, puis son prix
  await scroll(440, 1300); await wait(600); await shot('magasins');
  await scroll(240, 900); await wait(500); await shot('magasins-2');
  await tapIn('#store-list', 'Oui', { exact: true, pause: 1800 }); await shot('confirme');
  ...
  if (await priceInput.count().catch(() => 0)) {
    ...
    await priceInput.type('1.30', { delay: 190 }).catch(() => {});
    await wait(600);
    await tapIn('#store-list', 'OK', { exact: true, pause: 2200 }); await shot('prix');
  ...
  await page.evaluate(() => {
    let el = document.getElementById('map-box');
  ...
  await ff(['-y', '-ss', trim.toFixed(3), '-i', webm, ...audio, '-t', dur.toFixed(2),
    '-vf', `scale=${OUTPUT_W}:-2:flags=lanczos`,
  ...
  await ff(['-y', '-ss', trim.toFixed(3), '-i', webm, ...audio, '-t', dur.toFixed(2),
    '-vf', 'scale=-2:1920:flags=lanczos,pad=1080:1920:(ow-iw)/2:0:0x100e0c', '-c:v', 'libx264', '-preset', 'slow',
  ...
  if (SMOOTH) {
    console.log('Interpolation vers 60 i/s… (long)');
    // mpdecimate d'abord : l'enregistreur écrit à cadence fixe et répète les
    // images quand le rendu n'a pas suivi. En les retirant, l'interpolation
    // travaille entre deux images réellement différentes — deux fois moins
    // d'images figées dans le résultat (mesuré : 25 % -> 14 %).
    const INTERP = 'mpdecimate=hi=200:lo=100:frac=0.05,' +
