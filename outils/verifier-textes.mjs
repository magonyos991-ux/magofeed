...
/* ============================================================================
   ...
   1. LES CLÉS ORPHELINES — une entrée dont le texte français n'apparaît plus
      nulle part dans le code. Soit la phrase a été réécrite (il faut renommer la
      clé), soit l'écran a disparu (on peut la retirer). C'est le symptôme qui
      coûte le plus cher : la traduction existe, elle est devenue inatteignable.
...
let ko = 0;
ko += bloc("CLÉS ORPHELINES", orphelines,
  "leur texte français n'existe plus dans le code : la traduction est inatteignable.");
...
if (!ko) console.log("\nla table correspond au code : rien à signaler.");
process.exit(ko ? 1 : 0);
